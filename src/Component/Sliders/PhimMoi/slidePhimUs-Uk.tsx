...
export default function SlideImage() {
    ...
    const Moviephim =  PhimUsUk?.data.items[0]
    ...
    return (
      ...
      >     
            ...
            {PhimUsUk?.data.items.map((item, index) => (
                <SwiperSlide key={index}>
                        <div className="flip-card3 ">
                            <div className="flip-card-inner3 duration-200">
                                {/* Mặt trước */}
                                <div  className="flip-card-front3 ">
                                    <img src={`${PhimUsUk.data.APP_DOMAIN_CDN_IMAGE}/${item.poster_url}`} className="rounded-lg max-[1441px]:w-100 w-75 h-50 object-cover "/>
                                </div>
                                {/* Mặt sau */}
