...
// import { DanhSachAPI, MovieAPI, QuocgiaAPI, TheloaiAPI, TheloaiMovieAPI } from '../Services/API';
// import { useParams, useSearchParams } from 'react-router-dom';
// import type { Danhsach, QuocGiaResponse, TheloaiResponse } from '../types/Phimtype';
...
// export default function DuyetTim() {
...
//   const [quocgiasearch , setQuocGiaSearch] = useState<QuocGiaResponse | null>(null);
//   const [theloaisearch , setTheLoaiSearch] = useState<TheloaiResponse | null>(null);
...
//         limit: 64,
//       };
//       const QuocgiaSearchData = await MovieAPI(
//         type_list,
...
//           params.year,
//           params.limit);
//           if (QuocgiaSearchData) {
//         setQuocGiaSearch(QuocgiaSearchData);
//       }
//       const TheloaiSearchData = await TheloaiMovieAPI(
//         type_list,
//          parseInt(currentPage) ,
//           params.sort_field,
//           params.sort_type,
...
//           params.year,
//           params.limit);
//           if (TheloaiSearchData) {
//         setTheLoaiSearch(TheloaiSearchData);
//       }
...
//             className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('country') === '' ? ' text-amber-200 border-1 border-gray-600' : ''}`}>Tất cả</button>
//             {quocgia.map((item, index) => (
//                <button 
//                onClick={() => updatedFilter('country', item.slug)}
//                className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('country')=== item.slug?  'text-amber-200 border-1 border-gray-600' : ''}`}
//                key={index}>
//                 <div>{item.name}</div>
//               </button>
//               ))}
