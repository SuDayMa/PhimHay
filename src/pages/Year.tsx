import type { Year, Theloai, QuocGia } from "../types/Phimtype";
import { useState , useEffect } from "react";
import { QuocgiaAPI, TheloaiAPI, YearAPI } from "../Services/API";
...
export default function Year () {
    ...
    const [quocgias, setQuocgias] = useState<QuocGia []>([])
    ...
    useEffect(() => {
        const YearData = async () => {
            try{
                setLoading(true)
                if(!type_list) return
                const theloaimovie = await TheloaiAPI()
                const quocgiamovie = await QuocgiaAPI()
                setTheloais(theloaimovie)
                setQuocgias(quocgiamovie)
    ...
    const [openfilter, setOpenfilter] = useState<number | null>(null);
      ...
      if (page < 1 || page > (yearmovie?.data.params.pagination.totalPages || 1)) {
        return;
      }
    ...
    const filter = (
  <div>
    <h2 className='text-2xl font-bold mb-4 text-[#ffff]'>Năm: {yearslug} </h2>
    <div className="w-[30%]">
      <div 
          ...
          <div className="grid grid-cols-20 max-[1441px]:grid-cols-18 max-[1300px]:grid-cols-12 max-[800px]:grid-cols-8 max-[500px]:grid-cols-4 gap-2">
              {years.map((year) => (
                <button
                  ...
                  className={`px-3 py-1 text-[#ffff]  rounded cursor-pointer text-sm ${yearslug === year.toString() ? 'text-yellow-200 border-1 border-gray-600' : ''}`}
                >
                  {year}
                </button>
              ))}
