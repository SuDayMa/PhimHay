...
export default function Danhsach() {
  ...
  const LoadingDanhSach = async () => {
    try {
      ...
      const DanhSachData = await DanhSachAPI(
        type_list,
  ...
  LoadingDanhSach();
   ...
   const [openfilter, setOpenfilter] = useState<number | null>(null);
      ...
      if (page < 1 || page > (danhsach?.data.params.pagination.totalPages || 1)) {
        return;
      }
  ...
  <div>
    ...
    <div className="w-[30%]">
      ...
      >
        <span className={` max-[500px]:text-[10px] ${openfilter === 1 ? '' : ''}`}>
        <Icon name="filter" />
        </span>
      ...
      <div className="py-4 rounded-lg border-gray-600  border-[0.1px] mb-4 duration-700">
        ...
        <div className='border-b border-dashed border-gray-700 pb-3 mb-3 flex gap-5'>
          ...
          <div className="flex gap-2 ">
            ...
            className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('sort_field') === 'modified.time' ? ' text-yellow-200 border-1 border-gray-600' : ''}`}>Thời gian cập nhật
            </button>
            <button
            onClick={() => updatedFilter('sort_field', '_id')}
            className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('sort_field') === '_id' ? ' text-yellow-200 border-1 border-gray-600' : ''}`}>ID Phim
            </button>
            <button
            onClick={() => updatedFilter('sort_field', 'year')}
            className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('sort_field')=== 'year' ? ' text-yellow-200 border-1 border-gray-600' : ''}`}>Năm phát hành
            </button>
          </div>
        </div>
        ...
        <div className='border-b border-dashed border-gray-700 pb-3 mb-3 flex gap-5'>
          ...
          <div className="grid grid-cols-4 max-[350px]:grid-cols-3 gap-2">
            ...
            className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('sort_lang') ? ' ' : 'text-yellow-200 border-1 border-gray-600'}`}>Tất cả</button>
            <button
            onClick={() => updatedFilter('sort_lang', 'vietsub')}
            className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('sort_lang')=== 'vietsub' ? ' text-yellow-200 border-1 border-gray-600' : ''}`}>
              vietsub
            </button>
            <button
            onClick={() => updatedFilter('sort_lang', 'thuyet-minh')}
            className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('sort_lang') === 'thuyet-minh' ? ' text-yellow-200 border-1 border-gray-600' : ''}`}>
              Thuyết Minh
            </button>
            <button
            onClick={() => updatedFilter('sort_lang', 'long-tieng')}
            className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('sort_lang') === 'long-tieng' ? ' text-yellow-200 border-1 border-gray-600' : ''}`}>
              Lồng Tiếng
            </button>
          </div>
        </div>
        ...
        <div className='border-b border-dashed border-gray-700 pb-3 mb-3 flex gap-5'>
          ...
          <div className="grid grid-cols-14 max-[1441px]:grid-cols-12 max-[1300px]:grid-cols-8 max-[800px]:grid-cols-7 max-[500px]:grid-cols-4 gap-2">
            ...
            className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('category') ? '' : 'text-yellow-200 border-1 border-gray-600'}`}>Tất cả</button>
            {theloai.map((item, index) => (
               <button 
               onClick={() => updatedFilter('category', item.slug)}
               className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('category') === item.slug ? 'text-yellow-200 border-1 border-gray-600' : ''}`}
               key={index}>
                <div>{item.name}</div>
              </button>
            ))}
        ...
        <div className='border-b border-dashed border-gray-700 pb-3 mb-3 flex gap-5'>
          ...
          <div className="grid grid-cols-12 max-[1300px]:grid-cols-8 max-[800px]:grid-cols-7 max-[500px]:grid-cols-3 gap-2">
            ...
            className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('country')  ? '' : 'text-yellow-200 border-1 border-gray-600'}`}>Tất cả</button>
            {quocgia.map((item, index) => (
               <button 
               onClick={() => updatedFilter('country', item.slug)}
               className={`px-3 py-1 text-center hover:text-amber-200 text- text-[#ffff] rounded cursor-pointer text-sm ${filtersParam.get('country')=== item.slug?  'text-yellow-200 border-1 border-gray-600' : ''}`}
               key={index}>
                <div>{item.name}</div>
              </button>
              ))}
        ...
        <div className='border-b border-dashed border-gray-700 pb-3 mb-3 flex gap-5'>
          ...
          <div className="grid grid-cols-21 max-[1441px]:grid-cols-18 max-[1300px]:grid-cols-12 max-[800px]:grid-cols-8 max-[500px]:grid-cols-4 gap-2">
            ...
            className={`px-3 py-1 text-[#ffff] hover:text-amber-200 rounded cursor-pointer text-sm ${filtersParam.get('year') ? '':'text-yellow-200 border-1 border-gray-600'}`}>Tất cả</button>
              {years.map((year) => (
                <button
                  key={year}
                  onClick={() => updatedFilter('year',year.toString())}
                  className={`px-3 py-1 text-[#ffff]  rounded cursor-pointer text-sm ${filtersParam.get('year') === year.toString() ? ' text-yellow-200 border-1 border-gray-600' : ''}`}
                >
                  {year}
                </button>
              ))}
        ...
        <>  
            ...
            {danhsach && danhsach?.data.items?.map((item, index) => {
              ...
              <div key={index} className='bg-[#676b6d44] p-4 rounded-lg mb-4 group-hover:w-[100px] group-hover:h-[500px] transition-all duration-300 hover:bg-gray-600'>
                <div className='flex'>
                  <img src={`${danhsach.data.APP_DOMAIN_CDN_IMAGE}/${item.poster_url}`} className="w-full h-[200px] mb-2 rounded-lg "/>
